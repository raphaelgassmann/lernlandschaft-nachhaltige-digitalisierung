# Midjourney Prompts – Data Jungle Learning Landscape

## Style Suffix (use for all prompts)

All prompts end with the same style suffix for consistent results:

```
--style raw --s 200
```

**Common style appendix** (append to every prompt):
...
## 1. Main Background – Jungle Map

### 1a. Top-Down Map View
...
### 1b. Panoramic Landscape (perspective view)
...
### 1c. Background Texture (subtle, as CSS background)
...
## 2. Station Illustrations

### 2a. The Device Clearing (Pillar 1: Devices & Lifecycle)
...
### 2b. The Cloud Spring (Pillar 2: Data & Infrastructure)
...
### 2c. The Code Camp (Pillar 3: Usage Behavior & Software Practice)
...
## 3. Decorative Section Icons

### 3a. Microlearning Icon (lightbulb/knowledge)
...
### 3b. Action/Setting Icon (wrench/tool)
...
### 3c. Reflection Icon (thought bubble/mirror)
...
### 4a. Horizontal Divider (vine divider)
...
### After Generation
1. **Background (1a/1b/1c):** Use as `background-image` in CSS, optionally with overlay for readability
2. **Station images (2a/2b/2c):** Insert as header image in the `.station-header` area
3. **Icons (3a-3d):** Scale to ~64x64px or 128x128px, use as `<img>` in section headers
4. **Decorations (4a/4b):** Use as CSS background or image element
...
### Image Optimization
- Export all images as **WebP** (best compression for web)
- Background: max. 1920x1080px, quality 80%
- Station headers: max. 1200x800px, quality 85%
- Icons: 256x256px, quality 90%
- Decorations: as needed, quality 80%
