...
| 4 | Bestenliste auf Supabase umbauen | `highscore.js` komplett auf Supabase REST API umgestellt (fetch statt localStorage), upsert via `merge-duplicates`, async/Promise-basiert, lokaler Cache als Fallback, `abschluss-feier.html` auf Promises angepasst, `firebase-config.js` gelöscht | `js/highscore.js`, `stations/abschluss-feier.html`, `js/firebase-config.js` (gelöscht) |
| 5 | Spieler sofort in Bestenliste anzeigen | `syncCurrentPlayer()` bei Expeditionsstart, Station-Complete und Challenge-Complete aufrufen; `hasAlreadySubmitted`/`markHighscoreSubmitted` entfernt; Abschluss-Feier vereinfacht | `js/progress.js`, `js/highscore.js`, `stations/abschluss-feier.html` |
